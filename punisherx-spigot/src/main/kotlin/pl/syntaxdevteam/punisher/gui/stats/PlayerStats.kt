...
object PlayerStatsService {
    ...
    fun getLastLocationString(uuid: UUID): String? {
        return try {
            ...
            val worldName = worldKey?.let { Bukkit.getWorld(it.key)?.name } ?: tag.getString("Dimension")
    ...
    fun getLastLocation(uuid: UUID): Location? {
        return try {
            ...
            val worldKey = NamespacedKey.fromString(tag.getString("Dimension"))
            val world = worldKey?.let { Bukkit.getWorld(it.key) } ?: Bukkit.getWorld(tag.getString("Dimension"))
