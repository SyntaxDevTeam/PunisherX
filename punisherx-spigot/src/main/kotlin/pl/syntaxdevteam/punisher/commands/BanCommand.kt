...
class BanCommand(private var plugin: PunisherX) : BasicCommand {
            ...
            if (args.isNotEmpty()) {
                if (args.size < 2) {
                    ...
                    if (punishmentId == null) {
                        ...
                        val playerProfile = Bukkit.createPlayerProfile(uuid, player)
                        val banList: ProfileBanList = Bukkit.getBanList(BanList.Type.PROFILE)
    ...
    override fun suggest(@NotNull stack: CommandSourceStack, @NotNull args: Array<String>): List<String> {
        ...
        return when (args.size) {
            ...
            3 -> plugin.messageHandler.getMessageStringList("ban", "reasons").filter { it.startsWith(input, ignoreCase = true) }
            else -> emptyList()
        }
    }
}
