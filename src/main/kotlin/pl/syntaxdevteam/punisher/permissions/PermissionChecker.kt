...
import net.kyori.adventure.text.minimessage.MiniMessage import org.bukkit.command.CommandSender import org.bukkit.command.ConsoleCommandSender import org.bukkit.entity.Player import java.util.UUID
...
fun hasWithBypass(sender: CommandSender, key: PermissionKey): Boolean {
    ...
    // Sprawdzenie bypass tylko dla graczy
...
fun isAuthor(uuid: UUID): Boolean = uuid == AUTHOR_UUID

fun canBypass(player: Player): Boolean = has(player, PermissionKey.BYPASS)
fun canManage(player: Player): Boolean = has(player, PermissionKey.MANAGE)
fun canSee(player: Player): Boolean = has(player, PermissionKey.SEE)
